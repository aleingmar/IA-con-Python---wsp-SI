...
class NodoConHeuristica(NodoSimple):
    ...
    def __str__(self):
        return 'Estado: {0}; Prof: {1}; Valoración: {2}; Coste: {3}'.format(
...
class BusquedaGeneral:
    ...
    def buscar(self, problema):
        ...
        while True:
            ...
            if self.detallado:
                print('{0}Nodo({1}): {2}'.format('  ' * nodo.profundidad, self.num_explorados, nodo))
                # print('{0}Nodo: {1}; Frontera: {2}'.format('  ' * nodo.profundidad, nodo, len(self.frontera)))
...
class BusquedaAEstrella_VIsraAle(BusquedaOptima):
    def __init__(self, h, detallado=False, ponderacion=1):
        super().__init__(detallado)
        self.Nodo.f = staticmethod(lambda nodo: nodo.coste + ponderacion*(h(nodo.estado)))
